...
BEGIN
   ...
   _self_ref_column_name := TG_ARGV[1];
    ...
    IF  (TG_OP = 'INSERT') THEN
        ...
        EXECUTE format('
        UPDATE %I SET %3$I = %3$I + 1 
        WHERE %3$I >= $1.%2$I
        AND %4$I IS NOT DISTINCT FROM $1.%4$I
        ', TG_TABLE_SCHEMA, TG_TABLE_NAME, _column_name, _self_ref_column_name) USING new;
    END IF;
    ...
    IF  (TG_OP = 'UPDATE') THEN
    	...
    	-- EXECUTE format('SELECT TRUE boolCASE WHEN SELECT SELECT ($1).%s::text', _col_name) USING new INTO  _is_lowered_in_order;
        IF _new_order < _old_order then
            EXECUTE format('UPDATE %1$I.%2$I SET %3$I = %3$I + 1 WHERE %3$I >= $1.%3$I AND %3$I < $2.%3$I AND %4$I IS NOT DISTINCT FROM $2.%4$I', TG_TABLE_SCHEMA, TG_TABLE_NAME, _column_name, _self_ref_column_name) using new, old, TG_RELID;
        else
            EXECUTE format('UPDATE %1$I.%2$I SET %3$I = %3$I - 1 WHERE %3$I <= $1.%3$I AND %3$I > $2.%3$I AND %4$I IS NOT DISTINCT FROM $2.%4$I', TG_TABLE_SCHEMA, TG_TABLE_NAME, _column_name, _self_ref_column_name) using new, old, TG_RELID;
        END IF;
    END IF;
    ...
    IF  (TG_OP = 'DELETE') THEN
        EXECUTE format('UPDATE %1$I.%2$I SET %3$I = %3$I + 1 WHERE %3$I > $1.%3$I AND %4$I IS NOT DISTINCT FROM $1.%4$I', TG_TABLE_SCHEMA, TG_TABLE_NAME, _column_name, _self_ref_column_name) using OLD;
    END IF;
   return new;
END;
$$ LANGUAGE plpgsql;
...
DECLARE
    ...
    tables text[] := array[__PLACEHOLDER_ORDERED_COLUMN_ARRAY__];
    t text[];
BEGIN
    ...
    ('
        ...
        ALTER TABLE %1$I DROP CONSTRAINT IF EXISTS "ordered_list";
        ...
        EXECUTE PROCEDURE __reorder_ordered_list(%2$I, %4$I);
        ', t[1], t[2], t[3], t[4]);
END
LOOP;
END;
